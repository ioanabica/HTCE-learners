...
class BaselineExperiment:
    ...
    def run_benchmarks_new(self, learner_names_list, methods_list,
                           X_source_shared_train, X_target_shared_train,
                           X_source_specific_train, X_target_specific_train,
                           W_source_train, W_target_train,
                           Y_source_train, Y_target_train,
                           X_target_specific_test, X_target_shared_test,
                           cate_test):
        experiment_data = []
        ...
        for method in methods_list:
            if method == 'target':
                ...
                for learner_name in learner_names_list:
                    ...
                    learner = get_learner(learner_name, X_size=X_train.shape[1],
                ...
                new_X_train_target = RadialGAN_Treatment(Train_X=Train_X, Train_T=Train_W, Train_Y=Train_Y,
                                                         alpha=1.0)
                ...
                for learner_name in learner_names_list:
                    log.info(f"Fitting learner {learner_name} with method {method}.")
                    learner = get_flex_transfer_learner(learner_name, X_shared_size=X_source_shared_train.shape[1],
                                                        X_source_specific_size=X_source_specific_train.shape[1],
                                                        X_target_specific_size=X_target_specific_train.shape[1],
                                                        binary_Y=(len(np.unique(Y_source_train)) == 2),
                                                        n_iter=self.n_iter)

                    learner.train(X_source_specific=X_source_specific_train,
                                  X_source_shared=X_source_shared_train,
                                  X_target_specific=X_target_specific_train,
                                  X_target_shared=X_target_shared_train,
                                  y_source=Y_source_train, y_target=Y_target_train,
                                  w_source=W_source_train, w_target=W_target_train)
    ...
    def run(
        ...
        if self.synthetic_simulator_type == 'linear':
            sim = SyntheticSimulatorLinear(X_source_specific_train, X_source_shared_train, X_target_specific_train,
                                           X_target_shared_train, seed=self.seed)
        else:
        ...
        W_source_train, Y_source_train, _, W_target_train, Y_target_train, _ = sim.simulate_dataset(
            X_source_specific_train, X_source_shared_train, X_target_specific_train, X_target_shared_train,
            binary_outcome=binary_outcome, treatment_assign='pred')
        W_source_test, Y_source_test, _, W_target_test, Y_target_test, PO_target_test = sim.simulate_dataset(
            X_source_specific_test, X_source_shared_test, X_target_specific_test, X_target_shared_test,
            binary_outcome=binary_outcome, treatment_assign='pred')

        cate_test = PO_target_test[1] - PO_target_test[0]

        experiment_data = self.run_benchmarks_new(learner_names_list, methods_list,
                                                  X_source_shared_train, X_target_shared_train,
                                                  X_source_specific_train, X_target_specific_train,
                                                  W_source_train, W_target_train,
                                                  Y_source_train, Y_target_train,
        ...
        metrics_df.to_csv(results_path / f"dataset_{dataset}_"
...
class POSharingExperiment(BaselineExperiment):
    ...
    def run(
        ...
        for po_sharing_scale in self.po_sharing_scales_list:
            ...
            if self.synthetic_simulator_type == 'linear':
                if self.sharing_across_domains:
                    sim = SyntheticSimulatorLinear(X_source_specific_train, X_source_shared_train,
                                                   X_target_specific_train,
                                                   X_target_shared_train, alpha=po_sharing_scale, seed=self.seed)
                else:
                    sim = SyntheticSimulatorLinear(X_source_specific_train, X_source_shared_train,
                                                   X_target_specific_train,
                                                   X_target_shared_train, beta=po_sharing_scale, seed=self.seed)
            else:
                raise Exception('Unknown simulator type.')
        ...
        all_metrics_df_all.to_csv(results_path / f"dataset_{dataset}_"
                                                 f"{self.synthetic_simulator_type}_"
                                                 f"binary_{binary_outcome}-seed{self.seed}.csv")
    ...
    def run(
        ...
        for selection_bias_source in self.selection_bias_source_list:
            for selection_bias_target in self.selection_bias_target_list:
                ...
                (X_source_specific_train, X_source_shared_train, X_target_specific_train,
                 X_target_shared_train) = X_train
                (X_source_specific_test, X_source_shared_test, X_target_specific_test, X_target_shared_test) = X_test
                ...
                W_source_train, Y_source_train, _, W_target_train, Y_target_train, _ = sim.simulate_dataset(
                    X_source_specific_train, X_source_shared_train, X_target_specific_train, X_target_shared_train,
                    binary_outcome=binary_outcome, treatment_assign='pred',
                    prop_scale_source=selection_bias_source, pred_scale_target=selection_bias_target, )
                W_source_test, Y_source_test, _, W_target_test, Y_target_test, PO_target_test = sim.simulate_dataset(
                    X_source_specific_test, X_source_shared_test, X_target_specific_test, X_target_shared_test,
                    binary_outcome=binary_outcome, treatment_assign='pred',
                    prop_scale_source=selection_bias_source, pred_scale_target=selection_bias_target, )
