...
def sample_source_and_target_datasets(
    ...
    elif num_features_shared != None:  # noqa
    ...
    assert num_features_source + num_features_shared + num_features_target < X.shape[1]  # type: ignore
    ...
    X_source_shared = X[all_indices_examples[:num_examples_source]][
        :, all_indices_features[num_features_source : num_features_source + num_features_shared]  # type: ignore
    ]
    ...
    X_target_specific = X[all_indices_examples[num_examples_source : num_examples_source + num_examples_target]][
        ...
        all_indices_features[
            num_features_source + num_features_shared : num_features_source + num_features_shared + num_features_target  # type: ignore
        ],
    ]
...
def load(
    ...
    if train_ratio == 1.0:
        return (X_source_specific, X_source_shared, X_target_specific, X_target_shared)  # type: ignore
    else:

        X_source_specific_train = X_source_specific[: int(train_ratio * X_source_specific.shape[0])]  # type: ignore
        X_source_specific_test = X_source_specific[int(train_ratio * X_source_specific.shape[0]) :]  # type: ignore

        X_source_shared_train = X_source_shared[: int(train_ratio * X_source_shared.shape[0])]  # type: ignore
        X_source_shared_test = X_source_shared[int(train_ratio * X_source_shared.shape[0]) :]  # type: ignore

        X_target_specific_train = X_target_specific[: int(train_ratio * X_target_specific.shape[0])]  # type: ignore
        X_target_specific_test = X_target_specific[int(train_ratio * X_target_specific.shape[0]) :]  # type: ignore

        X_target_shared_train = X_target_shared[: int(train_ratio * X_target_shared.shape[0])]  # type: ignore
        X_target_shared_test = X_target_shared[int(train_ratio * X_target_shared.shape[0]) :]  # type: ignore
