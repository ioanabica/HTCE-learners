...
class SyntheticSimulatorBase:
    ...
    def simulate_dataset(
        self, X_source_specific, X_source_shared, X_target_specific, X_target_shared,
        ...
        if self.noise:
            error_source = (
                torch.empty(len(X_source_shared)).normal_(std=self.err_std).squeeze().detach().cpu().numpy())
            error_target = (
                torch.empty(len(X_target_shared)).normal_(std=self.err_std).squeeze().detach().cpu().numpy())
...
class SyntheticSimulatorLinear(SyntheticSimulatorBase):
    def __init__(
        self,
        X_source_specific, X_source_shared, X_target_specific, X_target_shared, alpha=0.5, beta=0.5,
        seed=42,
    ) -> None:
        super(SyntheticSimulatorLinear, self).__init__(seed=seed)

        scale_factor = 10
        self.po_sharing_source = np.random.uniform(-scale_factor, scale_factor, size=(
            X_source_specific.shape[1] + X_source_shared.shape[1]))
        self.po_sharing_target = np.random.uniform(-scale_factor, scale_factor, size=(
            X_target_specific.shape[1] + X_target_shared.shape[1]))
    ...
    def predict_Y0(self, X_source_specific, X_source_shared, X_target_specific, X_target_shared):
        ...
        po_sharing_source = np.dot(np.concatenate([X_source_shared, X_source_specific], axis=1),
                                   self.po_sharing_source) / \
        ...
        po_sharing_target = np.dot(np.concatenate([X_target_shared, X_target_specific], axis=1),
                                   self.po_sharing_target) / \
