...
def RadialGAN_Treatment(Train_X, Train_T, Train_Y, alpha):
    ...
    for it in tqdm(range(20000)):
        ...
        _, D_loss_curr = sess.run([D_solver, D_loss], feed_dict={X1: X1_mb, t1: t1_mb, y1: y1_mb, X2: X2_mb, t2:t2_mb, y2: y2_mb})
        _, G1_loss_curr = sess.run([G1_solver, G1_loss], feed_dict={X1: X1_mb, t1: t1_mb, y1: y1_mb, X2: X2_mb, t2:t2_mb, y2: y2_mb})
        _, G2_loss_curr = sess.run([G2_solver, G2_loss], feed_dict={X1: X1_mb, t1: t1_mb, y1: y1_mb, X2: X2_mb, t2:t2_mb, y2: y2_mb})
