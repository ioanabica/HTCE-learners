...
from contrib.catenets.models.torch.base import (
    DEVICE,
    BaseCATEEstimator,
    BasicNet,
    PropensityNet,
...
class SNet(BaseCATEEstimator):
    ...
    def __init__(
        ...
        self._reps_mu0 = RepresentationNet(
            n_unit_in, n_units=n_units_r_small, n_layers=n_layers_r, nonlin=nonlin, batch_norm=batch_norm
        )
            ...
            for idx in range(2):
                self._po_estimators.append(
                    BasicNet(
                        ...
                        n_units_r + n_units_r_small + n_units_r_small,  # (reps_c, reps_o, reps_mu{idx})
                        binary_y=binary_y,
    ...
    def train(
        ...
        for i in range(self.n_iter):
            ...
            for b in range(n_batches):
                ...
                batch_loss = self.loss(y0_preds, y1_preds, prop_preds, discrepancy, y_next, w_next)
            ...
            if self.early_stopping or i % self.n_iter_print == 0:
                with torch.no_grad():
                    y0_preds, y1_preds, prop_preds, discrepancy = self._step(X_val, w_val)
                    val_loss = self.loss(y0_preds, y1_preds, prop_preds, discrepancy, y_val, w_val).detach().cpu()
    ...
    def _step(self, X: torch.Tensor, w: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
