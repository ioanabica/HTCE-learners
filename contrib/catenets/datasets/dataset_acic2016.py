...
def get_acic_covariates(fn_csv: Path, keep_categorical: bool = False, preprocessed: bool = True) -> np.ndarray:
...
def preprocess_simu(
    ...
    X = get_acic_covariates(fn_csv, keep_categorical=keep_categorical, preprocessed=preprocessed)
    ...
    gamma = np.random.choice(coeffs_, size=n_cov, replace=True, p=[1 - prop_gamma, prop_gamma])
    omega = np.random.choice([0, 1], replace=True, size=n_cov, p=[prop_omega, 1 - prop_omega])
    ...
    if sp_nonlin > 0:
        ...
        if inter:
            ...
            beta_inter = np.random.choice(coefs_sq, size=n_inter, replace=True, p=[1 - sp_nonlin, sp_nonlin])
            omega = np.random.choice([0, 1], replace=True, size=n_inter, p=[prop_omega, 1 - prop_omega])
...
def get_acic_orig_filenames(data_path: Path, simu_num: int) -> list:
    return sorted(glob.glob((data_path / ("data_cf_all/" + str(simu_num) + "/zymu_*.csv")).__str__()))
...
def preprocess_acic_orig(
    ...
    if not random_split:
        ...
        (
            ...
            mu_1_train,
            mu_1_test,
        ) = train_test_split(X, y, w, mu_0, mu_1, test_size=1 - train_size, random_state=i_exp)
...
def preprocess(
    ...
    **kwargs: Any,
    ...
    if not original_acic_outcomes:
        ...
        return preprocess_acic_orig(fn_csv=fn_csv, preprocessed=preprocessed, data_path=data_path, **kwargs)
...
def load(
    ...
    **kwargs: Any,
    ...
    if preprocessed:
        ...
        download_if_needed(arch, file_id=FILE_ID, unarchive=True, unarchive_folder=data_path)

        csv = data_path / "data_cf_all/x.csv"
    log.debug(f"load dataset {csv}")

    return preprocess(
        csv, data_path=data_path, preprocessed=preprocessed, original_acic_outcomes=original_acic_outcomes, **kwargs
    )
